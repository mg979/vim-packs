...
__vpacks_completions()
{
	...
	if [ "${#COMP_WORDS[@]}" == "2" ]; then
		COMPREPLY=($(compgen -W "list update dirty verbose web git exe log logs move2opt move2start remotes fetch graph helptags restore backup terminal install" "${COMP_WORDS[1]}"))
		...
		if [ "${COMP_WORDS[1]}" == "restore" ]; then
			...
			COMPREPLY=($(compgen -W "errors clear clearall $packdirs $packs" "${COMP_WORDS[2]}"))
		else
			local packdirs=$(printf "%s\n" $vimdir/pack/* | grep -Ev 'backups|logs' | sed "s@$vimdir/pack/@@g")
			local packs=$(printf "%s\n" $vimdir/pack/*/*/* | grep -E 'start|opt' | sed "s@$vimdir/pack/[^/]\+/[^/]\+/@@g")
